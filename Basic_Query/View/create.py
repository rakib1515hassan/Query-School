...
from Basic_Query.forms import Query_CodeForm
...
def Query(request):
    return render(request, 'Query/basic_query.html')
...
def Query_CodeView(request):
    if request.method == 'POST' and request.headers.get('x-requested-with') == 'XMLHttpRequest':
        form = Query_CodeForm(request.POST)
        if form.is_valid():

            # Access and print form data
            # query_no = form.cleaned_data['query_no']
            # title = form.cleaned_data['title']
            # SQL_query = form.cleaned_data['SQL_query']
            # ORM_query = form.cleaned_data['ORM_query']

            instance = form.save()
            # Optionally, you can send a success response with JSON
            return JsonResponse({'success': 'Data saved successfully!'})
        
        else:
            # Send an error response with JSON, including form errors
            errors = form.errors.get('query_no')  # Get specific errors for 'query_no'
            if errors:
                return JsonResponse({'error': errors[0]}, status=400)  # Return the first error
            else:
                return JsonResponse({'error': 'Invalid form data.'}, status=400)
    else:
        form = Query_CodeForm()

    return render(request, 'Data_Insurt\query_code_add.html', {'form': form})